...
class SaeServer:
    ...
    def _setup_routes(self):
        ...
        def explain(data: TextInput):
            # Tokenize input
            inputs = self.tokenizer(data.text, return_tensors="pt", add_special_tokens=True)
            tokens = self.tokenizer.convert_ids_to_tokens(inputs["input_ids"][0])
            
            # Get token embeddings
            with torch.no_grad():
                token_embs = self.embedding_model(**inputs).last_hidden_state.squeeze(0)
            ...
            feature_strengths = {str(i): max(map(abs, s)) for i, s in enumerate(feature_scores)}
            
            # Reformat feature_scores as {feature_id: [score, score, ...]}
            feature_scores_dict = {str(i): s for i, s in enumerate(feature_scores)}
